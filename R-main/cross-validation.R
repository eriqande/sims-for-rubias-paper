...
# first, we need to fool the bootstrap rho function into thinking
# that we had a mixture data set tacked onto our reference data that
# had 1000 fish in it.  We do that here.  The actual genetic composition
# of those individuals is not important.
...
# now we need to compute the repunit posterior means and we want to make
# sure that they are in the same order that bootstrap_rho will be expecting
# them.  Hence we make a factor out of Hass_new$repunit so that when the 
# means are summarized, they are listed in the right order.
Hass_new$repunit <- factor(Hass_new$repunit, levels = unique(D$repunit))

# summarise the repunit proportions each iteration
rho_mcmc <- Hass_new %>%
...
# then, given the estimate of the repunit proportion each time, and 
# the estimate of the collections, do parametric bootstrapping using those estimated 
# collection proportions to obtain corrected estimates of the repunit proportions
...
# apparently renaming the methods again...
